...
class SonosTrayApp(ctk.CTk):
    def __init__(self):
        ...
        # 1. GROUP SELECTOR
        self.groups_card = self.create_card()
        ...
        self.track_label = ctk.CTkLabel(txt_box, text="Lade...", font=ctk.CTkFont(size=16, weight="bold"), anchor="w", wraplength=210, justify="left")
        self.track_label.pack(fill="x")
        self.artist_label = ctk.CTkLabel(txt_box, text="Lade...", font=ctk.CTkFont(size=13), anchor="w", wraplength=210, text_color="#bbbbbb", justify="left")
        ...
        self.autostart_card = self.create_card()
    ...
    def create_card(self):
        return ctk.CTkFrame(self.main_container, fg_color=CARD_BG, corner_radius=CORNER_RADIUS_INNER, border_width=1, border_color=CARD_BORDER)
    ...
    def setup_controls(self, parent):
        ...
        self.pause_btn = ctk.CTkButton(box, text="⏸", command=lambda: self.control_action("pause"), fg_color=BTN_DEFAULT, **btn_cfg)
    ...
    def check_autostart_status(self):
        """Prüft in der Registry, ob der Autostart aktiv ist."""
        try:
    ...
    def toggle_autostart(self):
        """Schreibt oder löscht den Autostart-Eintrag in der Registry."""
        ...
        try:
            ...
            if self.autostart_var.get():
                winreg.SetValueEx(key, "SonosTrayControl", 0, winreg.REG_SZ, app_path)
            else:
                try:
                    winreg.DeleteValue(key, "SonosTrayControl")
                except FileNotFoundError:
                    pass
            winreg.CloseKey(key)
        except Exception as e:
            print(f"Fehler beim Ändern des Autostarts: {e}")
    ...
    def rebuild_dynamic_sections(self):
        try:
            ...
            for p in active_g.members:
                ...
                m_btn = ctk.CTkButton(row, text="🔇", width=32, height=32, fg_color="transparent", 
                                      hover_color="#2a2a2b", font=ctk.CTkFont(size=18),
                                      command=lambda pl=p: self.toggle_mute(pl))
            ...
            for p in all_p:
                ...
                var = ctk.BooleanVar(value=is_in)
                chk = ctk.CTkCheckBox(self.group_ui_list, text=p.player_name, variable=var, 
                                      command=lambda pl=p: self.toggle_group_membership(pl),
        ...
        try:
            ...
            if len(self.group_list_frame.winfo_children()) != len(groups):
                ...
                for g in groups:
                    ...
                    btn = ctk.CTkButton(self.group_list_frame, text=g.coordinator.player_name, height=24, 
                                        fg_color=ACTIVE_BLUE if u_id == self.selected_group_uid else BTN_DEFAULT,
                                        corner_radius=6, width=60, command=lambda u=u_id: self.select_group(u))
                    btn._sonos_uid = u_id
                    btn.pack(side="left", padx=2)
            else:
                for btn in self.group_list_frame.winfo_children():
                    if hasattr(btn, '_sonos_uid'):
                        btn.configure(fg_color=ACTIVE_BLUE if btn._sonos_uid == self.selected_group_uid else BTN_DEFAULT)
            ...
            if active_g:
                ...
                for name, w in self.room_vol_widgets.items():
                    ...
                    w["slider"].set(p_fresh.volume)
                    w["mute_btn"].configure(text_color=MUTE_RED if p_fresh.mute else "#FFFFFF")
        except: pass
    ...
    def load_art(self, url, coord):
        try:
            if url.startswith('/'): url = f"http://{coord.ip_address}:1400{url}"
    ...
    def control_action(self, a):
        if a == "play": self.play_btn.configure(fg_color=ACTIVE_BLUE)
            ...
            is_active = self.shuffle_btn.cget("text_color") == ACTIVE_BLUE
            self.shuffle_btn.configure(text_color="#FFFFFF" if is_active else ACTIVE_BLUE)
        elif a == "repeat":
            is_active = self.repeat_btn.cget("text_color") == ACTIVE_BLUE
            self.repeat_btn.configure(text_color="#FFFFFF" if is_active else ACTIVE_BLUE)
        ...
        def t():
            ...
            if a == "next": coord.next()
                ...
                try: coord.play_mode = nm
                except: pass
        threading.Thread(target=t, daemon=True).start()

    def toggle_mute(self, p):
        if p.player_name in self.room_vol_widgets:
            btn = self.room_vol_widgets[p.player_name]["mute_btn"]
            is_muted = btn.cget("text_color") == MUTE_RED
            btn.configure(text_color="#FFFFFF" if is_muted else MUTE_RED)
        threading.Thread(target=lambda: self.controller.toggle_mute_player(p), daemon=True).start()

    def update_window_height(self):
        self.update_idletasks()
        h = self.outer_frame.winfo_reqheight() + 4
...
def start_tray():
    ...
    def run_tray():
        menu = pystray.Menu(item("Öffnen", lambda: root.after(0, lambda: (root.deiconify(), position_window(root), root.focus_force())), default=True),
                            item("Beenden", lambda i, m: (i.stop(), root.after(0, root.quit))))
        pystray.Icon("Sonos", tray_img, "Sonos", menu, 
                     action=lambda: root.after(0, lambda: (root.deiconify(), position_window(root), root.focus_force()))).run()
